...
```{r}
ggsave("../figures/DepthsMeasurePredictActivePairedSample.png", plot = pairedCombo, width = 6, height =3, units = c("in"), dpi=600, bg = "white" )
```

```{r}
set.seed(29)
( sample = datacubePred1 %>% 
  slice_sample(n = 30) %>%
    mutate(paired = row_number()) %>%
    dplyr::select(plotId, siteId, paired, msrType, msrDepth, predDepth) %>%
    pivot_longer(
...
```{r}
( pairedCombo2 =  cowplot::plot_grid(paired2, paired2, ncol=2,  label_size = 12)  ) 
```

```{r}
ggsave("../figures/DepthsMeasurePredictActivePairedSample1Plot.png", plot = pairedCombo2, width = 6, height =3, units = c("in"), dpi=600, bg = "white" )
```