...
```{r}
aggregate2 = datacubePred %>% 
     mutate(tsf = year-fireYr) %>%
  group_by(resName, submitNm, distur, paired, fireYr, year) %>%
  summarise(avgPredDepth = mean(predDepth),
            tsf = mean(tsf)) 

aggregate2$tsf[aggregate$distur == "unburned"] <- -9999
aggregate2
aggregate3 = aggregate2 %>% filter(meanTSF < 1000) 
aggregate3
```


```{r}
( plotMeanError <- ggplot(aggregate2, aes(x = tsf, y=avgPredDepth, color=distur, group=tsf)) +
...
# Repeat measures Time since fire
