...
```{r}
ecoPalette = c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99', '#CC6677', '#DDCC77')
```
...
( overviewMap = ggplot() +
  ...
  scale_fill_manual(values = permafrost_palette, labels  = c("continuous", "discontinuous", "sporatic"),
                      name = "Permafrost") +
 geom_sf(data=artic_pp, fill=NA, colour = "#2F4F4F", lwd=.8, linetype="dashed") +
   
  new_scale_fill() +
   geom_sf(data = wwf, aes(fill = ECO_NAME, color=ECO_NAME, size=prct),  pch=21, stroke =1, alpha=.65) +
...
```{r}
( ecozoneLegendR = ggplot() +
     geom_sf(data = nh_pp, fill = "#F9F6EE", colour="#A9AB9D") +
   
 new_scale_fill() +
   geom_sf(data = wwf, aes(fill = ECO_NAME, color=ECO_NAME),  pch=21, stroke =1.8, alpha=.65) +
  geom_sf_text(data = wwf, aes(label = prct), size=2) +
...
legReco <- get_legend(ecozoneLegendR)
...
ggsave("../figures/EcozoneLegendRight.png", plot = ecoRight, dpi = 600,  bg='white')
```

```{r}
( ecozoneLegendB = ggplot() +
     geom_sf(data = nh_pp, fill = "#F9F6EE", colour="#A9AB9D") +
...
```{r eval=FALSE, include=FALSE}
ggsave("../figures/PermafrostLegndRight.png", plot = pfRight, dpi = 600,  bg='white')
```

# Cowplot
```{r}
legend_permafrostB = cowplot::get_legend(permafrostLegendB)
legend_permafrostR = cowplot::get_legend(permafrostLegendR)
legendEcoR = cowplot::get_legend(ecozoneLegendR)
legendEcoB = cowplot::get_legend(ecozoneLegendB)
```
