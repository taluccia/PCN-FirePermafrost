...
---
From PCN-Fire-Sysnthesis "Manuscript Map"
...
ggplot() +
 ...
 geom_sf(data=artic_pp, fill=NA, colour = "#2F4F4F", lwd=1, linetype="dashed") +
  new_scale_fill() +
  geom_sf(data = datacube_pp, aes(fill = submitNm, color=submitNm)) +
...
ggplot() +
 ...
 geom_sf(data=artic_pp, fill=NA, colour = "#2F4F4F", lwd=1, linetype="dashed") +
   ...
   #geom_point(data = wwf, aes(x=x, y=y), shape=25) +
  geom_sf(data = wwf, aes(color=ECO_NAME), shape=25) +
  geom_point(data = wwf, aes(x, y, color='yellow')) +
  #new_scale_fill() +
 # geom_sf(data = datacube_pp, aes(fill = ECO_NAME, color=ECO_NAME)) +
  coord_sf(xlim = st_coordinates(disp_window)[,'X'], ylim = st_coordinates(disp_window)[,'Y']) +
    #coord_sf(xlim = disp_win_coord[,'X'], ylim = disp_win_coord[,'Y']) +
  #coord_sf(xlim = c(-4002324, 3512498), ylim = c(-3008650,  4002452) ) +
  theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.5), 
...
( overviewMap = ggplot() +
  ...
  geom_sf(data=artic_pp, fill=NA, colour = "#A9AB9D", lwd=1, linetype="dashed") +
    scale_fill_manual(values = permafrost_palette4, labels  = c("Continuous", "Discontinuous", "Sporadic"),
                      name = "Permafrost") +
    scale_color_manual(values = permafrost_palette4, labels  = c("Continuous", "Discontinuous", "Sporadic"),
                      name = "Permafrost") +
  new_scale_fill() +
   new_scale_color() +
...
theme(panel.grid.major = element_line(color = "honeydew4", linetype = "solid", size = 0.3), 
        panel.background = element_rect(fill = "#C5D8D7")) +
  theme(plot.margin=unit(c(t=0.5, r=1.5, b=0.5,l= 1.5), "mm")) + 
  theme(legend.position = "none", 
        axis.title.x = element_blank(), 
        axis.title.y = element_blank())
## Create legends

### Ecozone Legend Right
```{r}
( ecozoneLegendRight = ggplot() +
     geom_sf(data = nh_pp, fill = "#F9F6EE", colour="#A9AB9D") +
   
 new_scale_fill() +
   geom_sf(data = wwf, aes(fill = ECO_NAME, color=ECO_NAME),  pch=21, stroke =1.8, alpha=.75) +
  geom_sf_text(data = wwf, aes(label = prct), size=2) +
...
### Ecozone Legend Bottom

```{r}
( ecozoneLegendBottom = ggplot() +
     geom_sf(data = nh_pp, fill = "#F9F6EE", colour="#A9AB9D") +
...
### Permafrost Legend Right

```{r fig.height=5, fig.width=6}
(
  permafrostLegendRight <- ggplot() +
     geom_sf(data = nh_pp, fill = "#F9F6EE", colour="#A9AB9D") +
...
### Get the legend from Maps
```{r}
legend_permafrostBottom = cowplot::get_legend(permafrostLegendBottom)
legend_permafrostRight = cowplot::get_legend(permafrostLegendRight)
legendEcoRight = cowplot::get_legend(ecozoneLegendRight)
legendEcoBottom = cowplot::get_legend(ecozoneLegendBottom)
```
...
legend1_right <- cowplot::plot_grid(legend_permafrostRight, align = "v", nrow = 2)

# 4.2 add second legend to grid, specifying its location
( legendRight <- legend1_right +
  ggplot2::annotation_custom(
    grob = legendEcoRight,
    xmin = 0.55, xmax = 0.55, ymin = 0.45, ymax = 0.45
  )
)
...
```{r}
# 4.1 setup legends grid
legend1_grid <- cowplot::plot_grid(legendEcoBottom, align = "v", nrow = 2)
...
## Grid map with legend Right
```{r fig.height=6, fig.width=6}
MapBottomLegend = ggdraw(xlim = c(0, 6), ylim = c(0, 6)) +
    draw_plot(overviewMap, x = 0, y = 0, width = 6, height = 6) + 
  draw_image("../images/legendBottom.PNG", scale = 0.85, x = .2, y = -0.1, width = 6, height = 1) 
```

### Save Image
```{r eval=FALSE, include=FALSE}
ggsave("../figures/PCNMapOverviewBottomLegend.png", plot = MapBottomLegend, dpi = 600, width = 6, height = 6, bg='white')
```
