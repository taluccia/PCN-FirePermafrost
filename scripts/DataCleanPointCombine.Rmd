...
show rows with na
```{r}
(allNa = dataCube %>% filter_all(any_vars(is.na(.))) )
```

```{r}
( latNA = dataCube %>% filter(is.na(lat)) )
unique(latNA$lastNm)
```
As of 1-31-2024 Natali missing 15 lat coordinates

```{r}
( lonNA = dataCube %>% filter(is.na(lon)) )
unique(lonNA$lastNm)
```
As of 1-31-2024 Natali missing 24 lat coordinates
```{r}
...
```{r}
(doyNA = dataCube %>% filter(is.na(msrDOY)) )
(unique(doyNA$lastNm))
```
...
```{r}
( dataCubePts = dataCube %>% drop_na(lat, lon) %>% df_to_sf() )
```
